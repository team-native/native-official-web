...
| `/admin/login` | 관리자 로그인·접근 권한 요청 |
| `/admin` | 프로젝트·공고·지원서·문의·관리자 승인 관리 |
...
2. `supabase/migrations`의 SQL 파일을 파일명 순서대로 SQL Editor에서 실행합니다.
3. `/admin/login`에서 `junpyokim0106@gmail.com`으로 계정을 만들면 최고관리자 권한이 자동으로 등록됩니다.
4. 다른 관리자는 같은 화면에서 권한을 요청하고, 최고관리자가 `/admin`의 `접근 승인` 메뉴에서 승인합니다.
